...
public:
	...
	void printTree1(std::ostream &out = std::cout) const {
        if (isEmpty()) {
            out << "Empty tree" << std::endl;
        } else {
            printTree1(root, out);
...
private:
	...
	void printTree1(BinaryNode *t, std::ostream &out) const {
        /// 前序遍历
        if (t != nullptr) {
			out << t->element << std::endl;  // 打印当前节点
            printTree1(t->left, out);  // 先打印左子树
            printTree1(t->right, out);  // 再打印右子树
        }
    }
